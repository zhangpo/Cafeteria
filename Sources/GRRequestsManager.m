...
- (void)setDelegate:(id<GRRequestsManagerDelegate>)delegate
{
    if (_delegate != delegate) {
        _delegate = delegate;
        _delegateRespondsToPercentProgress = [_delegate respondsToSelector:@selector(requestsManager:didCompletePercent:maximumSize:forRequest:)];
    }
}
...
- (void)percentCompleted:(float)percent maximumSize:(float)maximumSize forRequest:(id<GRRequestProtocol>)request
{
    if (_delegateRespondsToPercentProgress) {
        [self.delegate requestsManager:self didCompletePercent:percent maximumSize:maximumSize forRequest:request];
    }
}
