...
- (NSData *)read:(GRRequest *)request
{
    ...
    if (bytesThisIteration > 0) {
        ...
        if ([request.delegate respondsToSelector:@selector(percentCompleted:maximumSize:forRequest:)]) {
            [request.delegate percentCompleted:request.percentCompleted maximumSize:request.maximumSize forRequest:request];
